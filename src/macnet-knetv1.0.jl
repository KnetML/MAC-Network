...
using JSON,JLD2,HDF5,Knet,Images
using Random,Printf
include("loss.jl")
...
function read_unit(w,mi₋1,ci,KBhw′,KBhw′′;train=false,tap=nothing)
    d,B,N      = size(KBhw′)
    d,BN       = size(KBhw′′)
    #KBhw'     : d x B x N  := w[3] * khw + w[4]
    #KBhw''    : d x BN     := reshape(w[5] * khw + w[7]),(d,BN))
    # if train
    #     mi₋1       = dropout(mi₋1,0.15)
    # end
    ...
    IcmKB      = reshape(w[8] * IcmKB_pre  .+ w[9],(B,N)) #eq r3.1.2
    #IcmKB     : B x N
...
function write_unit(w,m_new,mi₋1,mj,ci,cj;train=false,selfattn=true,gating=true,tap=nothing)
    d,B        = size(m_new)
    #mnew      : d x B
    T          = length(mj)
    #mj        : d x BT
    ...
    !selfattn && return mi
    ciproj     = w[6] * ci .+ w[7]
    ci_3d      = reshape(ciproj,d,B,1) #add linear layer before!!!!
    #ci_3d     : d x B x 1
    cj_3d      = reshape(hcat(cj...),d,B,T)
    #cj_3d     : d x B x T
    ...
    #sa'       : 1 x B x T
    mj_3d      = reshape(hcat(mj...),(d,B,T))
    #mj_3d     : d x B x T
    mi_sa      = reshape(sum(sa′ .* mj_3d;dims=3),(d,B)) #eq w2.2
    #m_sa      : d x B
    mi′′       = w[5] * mi_sa .+ mi   #eq w2.3
    #mi′′      : d x B

    !gating && return mi′′

    σci′       = sigm.(w[8] * ci .+ w[9])  #eq w3.1
    #σci′      : 1 x B
...
function forward_net(w,r,qs,batchSizes,xS,xB,xP;answers=nothing,p=12,selfattn=false,gating=false,tap=nothing)
    ...
    if train
          KBhw_2d   = dropout(KBhw_2d,0.15)
    end
    KBhw′_pre     = w[17]*KBhw_2d .+ w[18] # look if it is necessary
    KBhw′′        = w[19]*KBhw′_pre .+  w[20]
    ...
    for i=1:p
        ...
        m_new  = read_unit(wmac[5:13],mi,ci,KBhw′,KBhw′′;train=train,tap=tap)
        mi     = write_unit(wmac[14:end],m_new,mi,mj,ci,cj;train=train,selfattn=selfattn,gating=gating)
        if selfattn
            push!(cj,ci); push!(mj,mi)
        end
        tap!=nothing && (tap["cnt"]+=1)
    end

    y = output_unit(w[end-7:end-2],q,mi;train=train)
...
function modelrun(w,r,opts,data,feats,o,wrun=nothing;train=false)
    ...
    for i=1:L
        ...
        if train
            grads,lss = lossfun(w,r,questions,batchSizes,xS,xB,xP;answers=answers,p=o[:p],selfattn=o[:selfattn],gating=o[:gating])
	    ...
	    total += B
            ...
            preds  = forward_net(w,r,questions,batchSizes,xS,xB,xP;p=o[:p],selfattn=o[:selfattn],gating=o[:gating])
